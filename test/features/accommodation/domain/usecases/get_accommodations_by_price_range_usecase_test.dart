...
void main() {
  late GetAccommodationsByPriceRangeUseCase usecase;
  late MockAccommodationRepository mockRepository;

  setUp(() {
    mockRepository = MockAccommodationRepository();
    usecase = GetAccommodationsByPriceRangeUseCase(repository: mockRepository);
  });

  const tMinPrice = 1000.0;
  const tMaxPrice = 2000.0;
  const tPage = 1;
  const tLimit = 12;
  ...
  group('GetAccommodationsByPriceRangeParams', () {
    test('should have correct props', () {
      const params = GetAccommodationsByPriceRangeParams(
        minPrice: tMinPrice,
        maxPrice: tMaxPrice,
        page: tPage,
        limit: tLimit,
      );
      expect(params.props, [tMinPrice, tMaxPrice, tPage, tLimit]);
    });

    test('two params with same values should be equal', () {
      const params1 = GetAccommodationsByPriceRangeParams(
        minPrice: tMinPrice,
        maxPrice: tMaxPrice,
        page: tPage,
        limit: tLimit,
      );
      const params2 = GetAccommodationsByPriceRangeParams(
        minPrice: tMinPrice,
        maxPrice: tMaxPrice,
        page: tPage,
        limit: tLimit,
